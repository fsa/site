...
В качестве имени пользователя и группы указаны `xray`. Необходимо создать такого пользователя или указать своего. Файл конфигурации должен быть расположен в файле `/etc/xray/config.json` для `xray.service`, либо в папке `/etc/xray/`, с уникальным именем, для `xray@.service`. Прежде чем создавать файл необходимо создать случайно сгенерированные параметры:
