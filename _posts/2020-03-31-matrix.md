...
Для обновления клиента Element на своём сервере использую скрипт, написанный одним из участников сообщества Matrix:
...
```bash
git clone https://github.com/MurzNN/element-web-update.git
```

При этом в папке, откуда была запущена команда (например, `/var/www/matrix/`), будет создана папка `element-web-update`. Файл .env необходимо создавать внутри неё. Там же вы найдёте пример данного файла. Обновление скрипта можно выполнять через `git pull`.
