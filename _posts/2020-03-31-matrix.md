...
то в будущем, возможно, появится необходимость перенести Synapse на другую машину. Имея отдельное доменное имя можно сократить
...
www A 1.1.1.1
matrix A 1.1.1.1
_matrix._tcp SRV 5 0 8448 matrix
```
...
Файлы логов доступа и ошибок размещены в стандартном месте. Если лог доступа не нужнен, укажите вместо пути значение off. Если просто закомментировать строки, то лог будет писаться в соответствии с общими настройками nginx. Отключить лог ошибок с помощью значения off нельзя!!!

При желании, можно вести логи с использованием journald, входящего в состав systemd. Для этого необходимо в качестве путей указать следущие настройки:
...
Начиная с версии matrix-synapse 1.0 не требуется каким либо-образом сообщать, что у вас используется matrix сервер, если ваш сервер отвечает по адресу домена 2 уровня и порту 8448. Если эти данные отличаются, то можно сообшить другим серверам где находится сервер. Сделать это можно двумя способами:
...
Она содержит номер порта (8448) и адрес сервера, который отвечает за сервер Matrix (matrix.example.com.). При использовании этого способа Matrix сервер должен использовать сертификат вашего домена 2 уровня (example.com).
...
Как указано в [документации matrix-synapse][3] записи в DNS используются только для старых версий сервера (0,34 или более старых). Использование их большого смысла не имеет, т.к. в любом случае, вы столкнётесь с необходимостью настройки вашего веб-сервера.
...
Параметр m.homeserver говорит клиентам о местоположении сервера Matrix. В этом же ответе можно указать сервер идентификации с помощью параметра m.indentity_server. Подробнее об этом можно прочитать [в документации к протоколу][5].
...
## Настройка ведения лога Synapse через journald

В некоторых случаях удобнее вести логи с использованием journald. Пример настроек:
...
Таким образом, при обновлении Synapse до текущей версии не будет необходимости производить слияние файла конфигурации с новой версией.
...
[1]: https://levans.fr/shrink-synapse-database.html Оптимизация базы данных (англ.)

[2]: https://github.com/MurzNN/element-web-update Git-репозиторий со скриптом обновления веб-клиента Element для сервера

[3]: https://github.com/matrix-org/synapse/blob/master/docs/MSC1711_certificates_FAQ.md#configuring-certificates-for-compatibility-with-synapse-100 Конфигурирование сертификатов при использовании Synapse 1.0 (eng)

[4]: https://federationtester.matrix.org/ Сайт для тестирования федерации с сервером matrix.org.

[5]: https://matrix.org/docs/spec/client_server/latest#id175 Документация по протоколу взаимодействия между клиентом и сервером