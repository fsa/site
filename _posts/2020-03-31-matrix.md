...
---
Эта заметка поможет вам установить свой собственный сервер сети Matrix. Во время чтения статьи вы узнаете как:
...
- легко перенести свой сервер на другой машину пользуясь возможностями nginx;
- настроить журналирование с использованием journald из состава systemd.

В заметке не рассматривается установка сервера с использованием систем контейнеризации.
...
Использование метода webroot позволяет обновлять сертификаты в автоматическом режиме. После обновления сертификатов certbot отправляет сигнал сервису nginx и он перезагружает данные сертификатов.
