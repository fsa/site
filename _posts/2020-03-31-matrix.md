...
###################################################################
# Script for check new version of Element from.
# https://api.github.com/repos/vector-im/element-web/releases/latest
# and download new version, if update is avaiable
#
# https://gist.github.com/fsa/9ffb85ed9b2b3146e78859e4f6d99f89
# Orig: https://gist.github.com/MurzNN/ee64f98ab2e71b886c41d55594e5dd9e
#
###################################################################
...
VERSION_LATEST=`curl -s https://api.github.com/repos/vector-im/element-web/releases/latest | jq -r '.name' | sed s/v//` || { echo "Error checking last Element version!"; exit 1; }

if ( [[ -z "$VERSION_LATEST" ]] || [ "$VERSION_LATEST" == "null" ] ); then
  echo "Error! Received bad version number from https://api.github.com/repos/vector-im/element-web/releases/latest: $VERSION_LATEST"
  exit
fi
...
if ( [ "$VERSION_INSTALLED" != "$VERSION_LATEST" ] ); then
  ...
  DL_URL=`curl -s https://api.github.com/repos/vector-im/element-web/releases/latest | jq -r '.assets[0].browser_download_url'`
  curl -L -o $DIRECTORY_TMP/element-latest.tar.gz $DL_URL || { echo "Error downloading element-latest.tar.gz"; exit 1; }
  tar -xf $DIRECTORY_TMP/element-latest.tar.gz -C $DIRECTORY_TMP/
  find $DIRECTORY_INSTALL/* -not -name 'config*.json' -delete
  rm -f $DIRECTORY_INSTALL/config.sample.json
  mv $DIRECTORY_TMP/element-v$VERSION_LATEST/* $DIRECTORY_INSTALL/
  rm -rf $DIRECTORY_TMP/element-v$VERSION_LATEST
