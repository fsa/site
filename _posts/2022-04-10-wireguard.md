...
Для тех, кто решился воспользоваться Firewalld. Сначала его необходимо установить. В некоторых системах, например, Fedora он уже установлен по умолчанию. В Ubuntu 20.04 LTS (текущая LTS на момент написания этих строк) может быть установлен с помощью
...
Теперь все пакеты, которые будут проходить из зоны `internal` в `external` будут подвергнуты NAT. Теперь на Wireguard клиенте можно прописать
