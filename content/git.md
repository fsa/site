...
lastmod: 2026-07-06 03:57:00 +0700
tags: [Git]
...
В версии git 2.55 (июль 2026 года) появился альтернативный вариант. Можно объединить несколько их в одну группу. Для примера, объединим `origin`, `gitlab` и `bitbucket` в группу `publish`:

```bash
git config remotes.publish "origin gitlab bitbucket"
```

Теперь можно передать локальный репозиторий в несколько внешних репозиториев с помощью одной команды `git push publish` или, конкретную ветку, `git push publish main`.
...
Теперь можно создать или отредактировать указанный в команде файл `~/.gitignore`. Имя и расположение файла может быть иным. Для примера исключим служебную папку среды разработки Visual Studio Code и папку `.stuff`, где мы будем располагать разные временные файлы (идея имени папки папки взята у Валентина Удальцова с Youtube канала [«PHP Point»](https://www.youtube.com/@PHPPoint)).
