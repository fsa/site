...
Пользователь вбивает в адресной строке мой сайт [tavda.org](https://tavda.org) и некоторые браузеры до сих пор пытаются зайти на HTTP его версию. Да, это не запрещено. Контентные сайты могут себе это позволить. Но тога все те, кто перехватывает ваш трафик может видеть, что вы читаете. Поэтому самым безопасным будет переадресовывать своих клиентов на безопасную версию сайта, которая расположена на HTTPS сервере, где весь обмен трафиком идёт через TLS шифрование.
