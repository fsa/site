...
tags: [IPv6]
description: IPv6 - протокол нового поколения, который никак не может победить IPv4.
---
...
Если вы администратор сети, то вы, также, можете организовать для своих пользователей сеть IPv6 в механизмом трансляции NAT64 (из IPv6 в IPv4). Это позволит построить сеть полностью только на IPv6, но при этом получать доступ к ресурсам, которые доступны только по IPv4. Подробнее об этом [я писал в заметке](/dns64). Из достоинств - упрощается настройка сети, потому что не нужно настраивать IPv4 сеть. Из недостатков - не всё может корректно работать. В некоторых случаях может потребоваться дополнительное ПО на клиентах, которое обеспечит возможность работы некоторых приложений, которые не готовы к переходу на IPv6.
